...
select max(salary) as MaxSalary, first_name,last_name from dept_emp de inner join departments dp on dp.dept_no = de.dept_no inner join salaries sl on sl.emp_no = de.emp_no inner join employees ep on ep.emp_no = de.emp_no where dept_name = 'Customer Service' group by dept_name;
/*dept_nameの指定を追加しました*/
/*エイリアスを利用しました。group by dept_nameを追加しました。*/
